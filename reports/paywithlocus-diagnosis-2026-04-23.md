...
## health_checks Breakdown

Production DB query not executed from this environment (atlas, not Railway). The predicted breakdown based on the gateway behavior:
...
- `classifyHealthStatus(400, ...)` → falls through to line 318-319: `healthStatus: 'degraded'`, `checkStatus: 'degraded'`.
- For the minority of GET endpoints (buildwithlocus), GET returns 404 → same degraded classification.

Expected `health_checks` distribution for paywithlocus:
...
| 2. **Parser** (`parseMppChallenge` bug) | Ruled out | No `WWW-Authenticate` header is returned, so the parser is never invoked. Tempo's header uses `Payment ` (exact match for `startsWith('Payment ')`) and parses correctly. |
...
This is a design choice by the operator, not a bug in our code. Our parser, probe shape, and rate limiting all function correctly.
